...
description: Use Codex through the subagent-orchestrator MCP server for review-only external CLI feedback in Synth Warp.
capability: "external-codex-review agent asset workflow"
...
- Use the `subagent-orchestrator` MCP server (project key **`synth-warp-subagent-orchestrator`**).
