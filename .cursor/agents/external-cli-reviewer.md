...
description: Coordinate review-only Codex and Gemini runs through subagent-orchestrator MCP for Synth Warp, including sequential panel-style comparisons.
...
- Use only the `subagent-orchestrator` MCP server for external CLI agents (project key **`synth-warp-subagent-orchestrator`**).
