...
fn plan_header_presentation(
    _billing_metadata: Option<&BillingMetadata>,
    _has_team: bool,
    _is_anonymous: bool,
) -> PlanHeaderPresentation {
    PlanHeaderPresentation {
        badge_label: Some("Local".to_string()),
        show_personal_upgrade: false,
    }
}
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        ctx.subscribe_to_view(&teams_page_handle, |_, _, event, ctx| match event {
            ...
            TeamsPageViewEvent::OpenWarpDrive => {}
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        let section = if section.is_hidden_in_local_only_build() {
            SettingsSection::Account
        } else {
            section
        };
    ...
    fn cycle_pages(&mut self, direction: CycleDirection, ctx: &mut ViewContext<Self>) {
        ...
        let stops = build_nav_stops(&self.nav_items, |section| {
            !section.is_hidden_in_local_only_build()
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        for (nav_index, nav_item) in self.nav_items.iter().enumerate() {
            match nav_item {
                SettingsNavItem::Page(section) => {
                    let section = *section;
                    if section.is_hidden_in_local_only_build() {
                        continue;
                    }
