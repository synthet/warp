...
fn arrow_down_from_account_with_collapsed_agents_lands_on_first_subpage() {
    ...
    // not skip over the Agents umbrella.
...
fn arrow_up_from_code_with_collapsed_agents_lands_on_last_subpage() {
    ...
    // Pressing Up from the Code umbrella lands on the collapsed Agents
...
fn arrow_down_from_expanded_last_subpage_leaves_umbrella() {
    ...
    // the Code umbrella (next top-level stop).
    let next = simulate_cycle(
        &nav_items,
        &stops,
        SettingsSection::ThirdPartyCLIAgents,
...
fn arrow_down_across_adjacent_collapsed_umbrellas() {
    ...
    // first Code subpage (Code umbrella auto-expands).
