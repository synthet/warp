...
#[test]
fn local_only_build_shows_local_badge_without_upgrade_cta() {
    let presentation = plan_header_presentation(None, false, true);

    assert_eq!(presentation.badge_label.as_deref(), Some("Local"));
    assert!(!presentation.show_personal_upgrade);
}

#[test]
fn warp_cloud_account_sections_are_hidden_in_local_only_build() {
    assert!(SettingsSection::BillingAndUsage.is_hidden_in_local_only_build());
    assert!(SettingsSection::Referrals.is_hidden_in_local_only_build());
