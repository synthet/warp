...
fn render_global_ai_toggle(
    ...
    // Unused in this fork: no anonymous sign-up CTA. Kept so the call site is unchanged.
    _sign_up_button: &MouseStateHandle,
    ...
    // Synth Warp is local-first: AI needs no Warp account, so never swap the
    // toggle for an anonymous sign-up CTA.
    row.add_child(
        Container::new(
            ui_builder
...
impl SettingsWidget for ApiKeysWidget {
    ...
    fn render(
        ...
        // Synth Warp is commercial-free: omit Warp Build / credit purchase upgrade CTAs.
